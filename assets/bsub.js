...
//             var sellingPlan = this._getSellingPlanFromOptions(groupId, selectedOptions);
            ...
            console.log(sellingPlanId ,groupId, 'sellingPlanId')
            this._setSellingPlanIdInput(widget, sellingPlanId);
          	let weeksLightBox = [];
          	let weeksWithoutLightBox= [];
         
            var options = $('#option-443f8160e8514d187603d97741c928a440bd9240-qb option' );
			      for (var optionData of options) {
                    	console.log(optionData.value, 'optionData')
                        let week = optionData.text .match(/\d/g);
                        week = week.join("");    
              		    weeksLightBox.push({sort:parseInt(week) , key:optionData.value ,value:optionData.text });
                 
            }
          	weeksLightBox.sort(function(a, b) {
          		return a.sort - b.sort;
        	});
          	
//           $( options[ 4 ] ).insertAfter( $( options[ 0 ] ) );
            $('#option-443f8160e8514d187603d97741c928a440bd9240-qb').find('option').remove().end()
            weeksLightBox.forEach((data , i)=> {
              $('#option-443f8160e8514d187603d97741c928a440bd9240-qb').append($("<option></option>")
                 .attr("value", data.key)
                 .text(data.value));
            ...
            // Without LighBox
            var options = $('#option-443f8160e8514d187603d97741c928a440bd9240 option' );
			      for (var optionData of options) {
                    	console.log(optionData.value, 'optionData')
                        let week = optionData.text .match(/\d/g);
                        week = week.join("");    
            ...
            $('#option-443f8160e8514d187603d97741c928a440bd9240').find('option').remove().end()
            weeksLightBox.forEach((data , i)=> {
              $('#option-443f8160e8514d187603d97741c928a440bd9240').append($("<option></option>")
                 .attr("value", data.key)
                 .text(data.value)); 
            })
        },
        ...
        _setSellingPlanIdInput: function (widget, sellingPlanId) {
            ...
            var variantId = this._getVariantId(widget);
          	console.log(sellingPlanIdInput , variantId ,'variantId')
        ...
        _getActiveSellingPlanId: function (widget, groupId) {
          	var activePlanInputEl = $(`#option-${groupId}-qb option:selected`).val();
            if (activePlanInputEl === undefined) {
              var activePlanInputEl = $(`#option-${groupId} option:selected`).val();
            }
          	console.log(activePlanInputEl , 'activePlanInputEl')
            if (!activePlanInputEl) {
                console.error(`BSUB: Could not find active plan ID for group ${groupId}.`);
            }
            return activePlanInputEl;
        },
